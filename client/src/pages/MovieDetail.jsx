  ...
  <div className="px-6 md:px-16 lg:px-40 pt-30 md:pt-50">
    ...
    <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
      ...
      <div className="relative flex flex-col gap-3 flex-1">
        ...
        <h1 className="text-4xl font-semibold max-w-96 text">{show.movie.title}</h1>
        <div className="flex items-center gap-2 text-gray-300">
          <StarIcon className="w-5 h-5 text-primary fill-primary"/>
          {show.movie.vote_average.toFixed(1)} User Rating
        </div>
        <p className="text-gray-400 mt-sm text-sm leading-tight max-w-xl">
          {show.movie.overview}
        </p>
        <p>
    ...
    {/* Cast Section - Outside the main flex container */}
    <div className="max-w-6xl mx-auto mt-20">
      <p className="text-lg font-medium">Your Favorite Cast</p>
      <div className="overflow-x-auto no-scrollbar mt-8 pb-4">
        <div className="flex items-center gap-4 w-max px-4">
          {show.movie.casts.slice(0,12).map((cast, index) => (
            <div key={cast.id || index} className="flex flex-col items-center text-center min-w-[80px]">
              <img 
                src={cast.profile_path} 
                alt={cast.name}
                className="rounded-full h-16 md:h-20 w-16 md:w-20 object-cover"
              />
